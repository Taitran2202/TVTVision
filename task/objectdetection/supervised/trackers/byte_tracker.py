...
#             self.xywh_to_cxcyah(self._xywh))
...
#         # # Lọc ra các đối tượng cần theo dõi dựa trên labels
#         # selected_inds = (labels == 14)
#         # scores = scores[selected_inds]
#         # bboxes = bboxes[selected_inds]
...
class STrack(BaseTrack):
    ...
    def __init__(self, xywh, score, label):
...
class ByteTracker(object):
    ...
    def update(self, scores, bboxes, labels):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.xyxy_to_xywh(xyxy), s, l) for
                          (xyxy, s, l) in zip(dets, scores_keep, labels_keep)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack(STrack.xyxy_to_xywh(xyxy), s, l) for
                                 (xyxy, s, l) in zip(dets_second, scores_second, labels_second)]
        else:
            detections_second = []
        r_tracked_stracks = [strack_pool[i]
                             for i in u_track if strack_pool[i].state == TrackState.Tracked]
