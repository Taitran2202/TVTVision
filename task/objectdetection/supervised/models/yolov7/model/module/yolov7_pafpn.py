...
class Yolov7PaFPN(nn.Module):
    def __init__(self,
        ...
        # top dwon
        # P5 -> P4
        self.reduce_layer_1 = Conv(
            c5, round(256*width), k=1, norm_type=norm_type, act_type=act_type)
        self.reduce_layer_2 = Conv(
            c4, round(256*width), k=1, norm_type=norm_type, act_type=act_type)
        ...
        # P4 -> P3
        self.reduce_layer_3 = Conv(
            round(256*width), round(128*width), k=1, norm_type=norm_type, act_type=act_type)
        self.reduce_layer_4 = Conv(
            c3, round(128*width), k=1, norm_type=norm_type, act_type=act_type)
        ...
        self.head_conv_1 = Conv(round(128*width), round(256*width),
                                k=3, s=1, p=1, act_type=act_type, norm_type=norm_type)
        self.head_conv_2 = Conv(round(256*width), round(512*width),
                                k=3, s=1, p=1, act_type=act_type, norm_type=norm_type)
        self.head_conv_3 = Conv(round(512*width), round(1024*width),
                                k=3, s=1, p=1, act_type=act_type, norm_type=norm_type)
        # output proj layers
