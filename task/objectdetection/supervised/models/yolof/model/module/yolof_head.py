...
class YOLOFHead(nn.Module):
    def __init__(self, cfg, in_dim, out_dim, num_classes, num_cls_head=1, num_reg_head=1, act_type='relu', norm_type='BN'):
        ...
        self.obj_pred = nn.Conv2d(
            self.reg_head_dim, 1 * self.num_anchors, kernel_size=3, padding=1)
        self.cls_pred = nn.Conv2d(
            self.cls_head_dim, num_classes * self.num_anchors, kernel_size=3, padding=1)
        self.reg_pred = nn.Conv2d(
            self.reg_head_dim, 4 * self.num_anchors, kernel_size=3, padding=1)
    ...
    def get_anchors(self, fmp_size):
        ...
        if self.fmp_size is not None and self.fmp_size == fmp_size:
            ...
            anchor_xy = torch.stack(
                [anchor_x, anchor_y], dim=-1).float().view(-1, 2) + 0.5
            # [HW, 2] -> [HW, 1, 2] -> [HW, KA, 2]
    ...
    def decode_boxes(self, anchor_boxes, pred_reg):
        ...
        pred_ctr_offset = torch.clamp(
            pred_ctr_offset, min=-self.ctr_clamp, max=self.ctr_clamp)
    ...
    def forward(self, x):
        ...
        normalized_cls_pred = normalized_cls_pred.permute(
        ...
        reg_pred = reg_pred.view(
            B, -1, 4, H, W).permute(0, 3, 4, 1, 2).contiguous()
