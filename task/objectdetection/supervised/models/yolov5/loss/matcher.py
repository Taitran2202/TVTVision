...
class Yolov5Matcher(object):
    ...
    def compute_iou(self, anchor_boxes, gt_box):
        ...
        anchors[..., :2] = anchor_boxes[..., :2] - \
            anchor_boxes[..., 2:] * 0.5  # x1y1
        anchors[..., 2:] = anchor_boxes[..., :2] + \
            anchor_boxes[..., 2:] * 0.5  # x2y2
    ...
    def iou_assignment(self, ctr_points, gt_box, fpn_strides):
        ...
        if iou_mask.sum() == 0:
            ...
            label_assignment_results.append(
                [grid_x, grid_y, xc_s, yc_s, level, anchor_idx])
        else:
    ...
    def __call__(self, fmp_sizes, fpn_strides, targets):
        ...
        for batch_index in range(bs):
            ...
            for gt_box, gt_label in zip(tgt_box, tgt_cls):
                ...
                keeps = np.maximum(
                    ratios, 1 / ratios).max(-1) < self.anchor_theshold

                if keeps.sum() == 0:
                    label_assignment_results = self.iou_assignment(
                        [xc, yc], gt_box, fpn_strides)
                else:
                    label_assignment_results = self.aspect_ratio_assignment(
                        [xc, yc], keeps, fpn_strides)
                ...
                for result in label_assignment_results:
                    ...
                    if off_x <= 0.5 and off_y <= 0.5:  # top left
                        grids = [(grid_x-1, grid_y),
                                 (grid_x, grid_y-1), (grid_x, grid_y)]
                    ...
                    for (i, j) in grids:
                        is_in_box = (j >= y1s and j < y2s) and (
                            i >= x1s and i < x2s)
                        is_valid = (j >= 0 and j < fmp_h) and (
                            i >= 0 and i < fmp_w)

                        if is_in_box and is_valid:
                            # obj
                            gt_objectness[level][batch_index,
                                                 j, i, anchor_idx] = 1.0
                            # cls
                            cls_ont_hot = torch.zeros(self.num_classes)
                            cls_ont_hot[int(gt_label)] = 1.0
                            gt_classes[level][batch_index, j,
                                              i, anchor_idx] = cls_ont_hot
                            # box
                            gt_bboxes[level][batch_index, j, i, anchor_idx] = torch.as_tensor([
                                                                                              x1, y1, x2, y2])

        # [B, M, C]
        gt_objectness = torch.cat([gt.view(bs, -1, 1)
                                  for gt in gt_objectness], dim=1).float()
        gt_classes = torch.cat([gt.view(bs, -1, self.num_classes)
                               for gt in gt_classes], dim=1).float()
        gt_bboxes = torch.cat([gt.view(bs, -1, 4)
                              for gt in gt_bboxes], dim=1).float()
