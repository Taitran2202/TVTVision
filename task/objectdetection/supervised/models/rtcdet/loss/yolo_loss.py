...
class YOLOLoss(object):
    def __init__(self, max_epoch, no_aug_epoch, loss_dfl_weight, loss_cls_weight,
                 loss_iou_weight, loss_box_aux, ema_update, center_sampling_radius,
                 topk_candidate, reg_max, device, num_classes=80):
        self.device = device
        self.num_classes = num_classes
        self.max_epoch = max_epoch
        self.no_aug_epoch = no_aug_epoch
        self.use_ema_update = ema_update
        self.reg_max = reg_max
        # ---------------- Loss weight ----------------
        self.loss_cls_weight = loss_cls_weight
        self.loss_iou_weight = loss_iou_weight
        self.loss_dfl_weight = loss_dfl_weight
        self.loss_box_aux = loss_box_aux
        # ---------------- Matcher ----------------
    ...
    def loss_classes(self, pred_cls, gt_score, gt_label=None, vfl=False):
        if vfl:
            ...
            focal_weight = alpha * pred_cls.sigmoid().pow(gamma) * (1 - gt_label) + \
                gt_score * gt_label
    ...
    def __call__(self, outputs, targets, epoch=0):
        ...
        for batch_idx in range(bs):
            ...
            if len(tgt_labels) == 0 or tgt_bboxes.max().item() == 0.:
                ...
                cls_target = cls_preds.new_zeros(
                ...
                # prepare cls targets
                assigned_labels = F.one_hot(
                    assigned_labels.long(), self.num_classes)
                assigned_labels = assigned_labels * assigned_ious.unsqueeze(-1)
                cls_target = assigned_labels.new_zeros(
                    (num_anchors, self.num_classes))
        ...
        if self.use_ema_update:
            normalizer = self.ema_update(
                "loss_normalizer", max(num_fgs, 1), 100)
        else:
            normalizer = num_fgs
        ...
        loss_dfl = self.loss_dfl(
            reg_preds_pos, box_targets, anchors_pos, strides_pos)
        ...
        if epoch >= (self.max_epoch - self.no_aug_epoch - 1) and self.loss_box_aux:
            ...
            # aux loss
            loss_box_aux = self.loss_bboxes_aux(
                delta_preds_pos, box_targets, anchors_pos, strides_pos)
