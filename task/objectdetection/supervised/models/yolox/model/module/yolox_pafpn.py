...
class YoloxPaFPN(nn.Module):
    def __init__(self, cfg, in_dims=[256, 512, 1024], out_dim=None):
        ...
        self.top_down_layer_1 = build_fpn_block(
            cfg, c4 + round(512*width), round(512*width))

        # P4 -> P3
        self.reduce_layer_2 = build_reduce_layer(
            cfg, round(512*width), round(256*width))
        self.top_down_layer_2 = build_fpn_block(
            cfg, c3 + round(256*width), round(256*width))

        # bottom up
        # P3 -> P4
        self.reduce_layer_3 = build_downsample_layer(
            cfg, round(256*width), round(256*width))
        self.bottom_up_layer_1 = build_fpn_block(cfg, round(
            256*width) + round(256*width), round(512*width))

        # P4 -> P5
        self.reduce_layer_4 = build_downsample_layer(
            cfg, round(512*width), round(512*width))
        self.bottom_up_layer_2 = build_fpn_block(cfg, round(
            512*width) + round(512*width), round(1024*width))
