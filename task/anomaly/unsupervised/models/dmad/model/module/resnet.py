    ...
    def forward(self, input_tensor, pe_reduction=4):
        ...
        if self.xx_channel is None:
            ...
            xx_range = (torch.arange(
                dim_y, dtype=torch.float32) / (dim_y - 1)).cuda()
            yy_range = (torch.arange(
                dim_x, dtype=torch.float32) / (dim_x - 1)).cuda()
        ...
        out = torch.cat([input_tensor, xx_channel, xx_channel_,
                        yy_channel, yy_channel_], dim=1)
        return out


class CoordConv2d(conv.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=False, r=4):
        super(CoordConv2d, self).__init__(in_channels,
        ...
        if pretrained:
            try:
                state_dict = torch.load(
                    "/data/liuwenrui/modelzoo/wide_resnet50_2-95faca4d.pth")
            except:
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, latents):
        ...
        # [B x D x H x W]
        return quantized_latents.permute(0, 3, 1, 2).contiguous(), commitment_loss * self.beta + embedding_loss
...
class BN_layer(nn.Module):
    def __init__(self,
        ...
        self.dilation = 1
        self.vq = vq
        self.catwidth = 1024+(16+16+16) * \
            block.expansion if self.vq else 1024*3
        ...
        if self.vq:
            ...
            self.conv1 = conv3x3((16+0) * block.expansion,
                                 16 * block.expansion, 2)
            ...
            self.conv3 = conv3x3((0+16) * block.expansion,
                                 16 * block.expansion, 2)
            self.bn3 = norm_layer(16 * block.expansion)
            self.conv4 = conv1x1((256) * block.expansion,
                                 256 * block.expansion, 1)
            ...
            self.conv1 = conv3x3((64) * block.expansion,
                                 128 * block.expansion, 2)
            self.bn1 = norm_layer(128 * block.expansion)
            self.relu = nn.ReLU(inplace=True)
            self.conv2 = conv3x3(128 * block.expansion,
                                 256 * block.expansion, 2)
            self.bn2 = norm_layer(256 * block.expansion)
            self.conv3 = conv3x3((128) * block.expansion,
                                 256 * block.expansion, 2)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        if self.vq:
            ...
            l1_ = self.relu(
                self.bn2(self.conv2(self.relu(self.bn1(self.conv1(l1))))))
...
class OffsetNet(nn.Module):
    def __init__(self, gamma=1, n_channel=3, bn=True):
        ...
        class Smooth_Loss(nn.Module):
            def __init__(self, channels=2, ks=7, alpha=1):
                ...
                self.filter = filter.view(
                    1, 1, ks, ks).repeat(1, channels, 1, 1)

            def forward(self, gen_frames):
                gen_frames = nn.functional.pad(
                    gen_frames, (self.ks // 2, self.ks // 2, self.ks // 2, self.ks // 2))
    ...
    def forward(self, x):
        ...
        offset1 = F.interpolate(
            offset1, (w, h), mode='bilinear', align_corners=True).permute(0, 2, 3, 1)
        offset2 = F.interpolate(
            offset2, (w, h), mode='bilinear', align_corners=True).permute(0, 2, 3, 1)
        offset1_ = F.interpolate(
            offset1_, (w, h), mode='bilinear', align_corners=True).permute(0, 2, 3, 1)
        offset2_ = F.interpolate(
            offset2_, (w, h), mode='bilinear', align_corners=True).permute(0, 2, 3, 1)

        gridY = torch.linspace(-1, 1, steps=h).view(1, -
                                                    1, 1, 1).expand(bs, w, h, 1)
        gridX = torch.linspace(-1, 1, steps=w).view(1,
                                                    1, -1, 1).expand(bs, w, h, 1)
        ...
        if self.training:
            transposed = F.grid_sample(F.grid_sample(
                out2, grid2_, align_corners=True), grid1_, align_corners=True)
