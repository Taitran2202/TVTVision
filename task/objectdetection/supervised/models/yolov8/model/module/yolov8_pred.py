...
class SingleLevelPredLayer(nn.Module):
    ...
    def init_bias(self):
        ...
        b.data.fill_(math.log(5 / self.num_classes /
                     (640. / self.stride) ** 2))
    ...
    def forward(self, cls_feat, reg_feat):
        ...
        # output dict
        outputs = {
            "pred_cls": cls_pred,            # List(Tensor) [B, M, C]
...
class MultiLevelPredLayer(nn.Module):
    def __init__(self,
        ...
        self.proj_conv = nn.Conv2d(
            self.reg_max, 1, kernel_size=1, bias=False).requires_grad_(False)
        self.proj_conv.weight.data[:] = nn.Parameter(
            proj_init.view([1, reg_max, 1, 1]))
    ...
    def forward(self, cls_feats, reg_feats):
        ...
        for level in range(self.num_levels):
            ...
            outputs = self.multi_level_preds[level](
            ...
            # tlbr -> xyxy
            x1y1_pred = outputs["anchors"][None] - \
                delta_pred[..., :2] * self.strides[level]
            x2y2_pred = outputs["anchors"][None] + \
                delta_pred[..., 2:] * self.strides[level]
...
def build_pred_layer(cls_dim, reg_dim, strides, num_classes, num_coords=4, num_levels=3, reg_max=16):
    pred_layers = MultiLevelPredLayer(
        cls_dim, reg_dim, strides, num_classes, num_coords, num_levels, reg_max)
