...
def build_model(cfg, device, size=None):
    ...
    checkpoint_path = os.path.join(
        cfg.TRAIN.base_weight, f'YOLOv4-{model_type}/best_model.pth')
    if os.path.exists(checkpoint_path):
        ...
        for key in checkpoint_state_dict:
            if key in model_state_dict and not key.startswith("cls_preds") and not key.startswith("non_shared_heads"):
