...
class FreeYOLO(nn.Module):
    def __init__(self, cfg, device, num_classes=20, conf_thresh=0.05, nms_thresh=0.6, topk=1000, no_decode=False):
        ...
        self.backbone, bk_dim = build_backbone(
            cfg=cfg, pretrained=cfg.pretrained)

        # neck
        self.neck = build_neck(
            cfg=cfg, in_dim=bk_dim[-1], out_dim=cfg.neck_dim)

        # fpn
        self.fpn = build_fpn(
            cfg=cfg, in_dims=cfg.fpn_dim, out_dim=cfg.head_dim)
    ...
    def inference_single_image(self, x):
        ...
        if self.no_decode:
            ...
            # [n_anchors_all, 4 + 1 + C]
            outputs = torch.cat(
                [reg_preds, obj_preds.sigmoid(), cls_preds.sigmoid()], dim=-1)
