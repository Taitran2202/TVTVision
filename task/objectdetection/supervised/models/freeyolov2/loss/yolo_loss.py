...
class YOLOLoss(object):
    def __init__(self,
        ...
        # loss
        self.cls_lossf = ClassificationLoss(cls_loss_name, reduction='none')
        self.reg_lossf = RegressionLoss(
            num_classes, reg_max - 1, self.use_dfl)
        # loss weight
    ...
    def __call__(self, outputs, targets):
        ...
        for batch_idx in range(bs):
            ...
            if len(tgt_labels) == 0 or tgt_boxs.max().item() == 0.:
                ...
                gt_score = cls_preds.new_zeros(
                    (1, num_anchors, self.num_classes))  # [1, M, C]
                ...
                (
                    ...
                    pd_scores=cls_preds[batch_idx:batch_idx + \
                                        1].detach().sigmoid(),
        ...
        # [BM,]
        fg_masks = torch.cat(fg_masks, 0).view(-1)
        gt_label_targets = torch.cat(
            gt_label_targets, 0).view(-1)                    # [BM,]
        gt_score_targets = torch.cat(
            gt_score_targets, 0).view(-1, self.num_classes)  # [BM, C]
        gt_bbox_targets = torch.cat(
            gt_bbox_targets, 0).view(-1, 4)                   # [BM, 4]
        ...
        gt_labels_one_hot = F.one_hot(
            gt_label_targets.long(), self.num_classes + 1)[..., :-1]
        loss_cls = self.cls_lossf(
            cls_preds, gt_score_targets, gt_labels_one_hot)

        # reg loss
        anchors = anchors[None].repeat(
            bs, 1, 1).view(-1, 2)                           # [BM, 2]
        strides = torch.cat(strides, dim=0).unsqueeze(
            0).repeat(bs, 1, 1).view(-1, 1)  # [BM, 1]
        # [BM, 1]
        bbox_weight = gt_score_targets[fg_masks].sum(-1, keepdim=True)
        # [BM, 4*(reg_max + 1)]
        reg_preds = reg_preds.view(-1, 4*self.reg_max)
        # [BM, 4]
...
class ClassificationLoss(nn.Module):
    def __init__(self, cls_loss_name, reduction='none'):
    ...
    def varifocalloss(self, pred_logits, gt_score, gt_label, alpha=0.75, gamma=2.0):
        focal_weight = alpha * pred_logits.sigmoid().pow(gamma) * (1 - gt_label) + \
            gt_score * gt_label
