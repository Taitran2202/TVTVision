    ...
    def __init__(
        ...
        try:
            self.permute_function = {
                0: F.linear, 1: F.conv1d, 2: F.conv2d, 3: F.conv3d}[self.input_rank]
        except KeyError:
        ...
        if global_affine_type == "SIGMOID":
            global_scale = 2.0 - \
                torch.log(torch.tensor([10.0 / global_affine_init - 1.0]))
        ...
        self.global_scale = nn.Parameter(torch.ones(
            1, self.in_channels, *([1] * self.input_rank)) * global_scale)
        ...
        if subnet_constructor is None:
            raise ValueError(
                "Please supply a callable subnet_constructor" "function or object (see docstring)")
