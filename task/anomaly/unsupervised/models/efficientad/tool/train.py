...
def training(model, trainloader, validloader, testloader, penaltyloader, criterion, optimizer, scheduler, num_training_steps: int = 1000, loss_weights: List[float] = [0.6, 0.4],
    ...
    st_losses_m = AverageMeter()
    ae_losses_m = AverageMeter()
    stae_losses_m = AverageMeter()
    ...
    while train_mode:
        ...
        for (images, _), (images_penalty, _) in zip(trainloader, penaltyloader):
            images = images.to(device, dtype=torch.float32)
            images_penalty = images_penalty['image'].to(
                device, dtype=torch.float32)

            data_time_m.update(time.time() - end)

            # predict
            loss_st, loss_ae, loss_stae = model(images, images_penalty)
            ...
            # log loss
            st_losses_m.update(loss_st.item())
            ae_losses_m.update(loss_ae.item())
            stae_losses_m.update(loss_stae.item())
            ...
            if use_wandb:
                wandb.log({
                    ...
                    'train_st_loss': st_losses_m.val,
                    'train_ae_loss': ae_losses_m.val,
                    'train_stae_loss': stae_losses_m.val
                },
                    step=step)
            ...
            if (step+1) % log_interval == 0 or step == 0:
                _logger.info(
                    'TRAIN [{:>4d}/{}] '
                    'Loss: {loss.avg:>6.4f} '
                    'ST Loss: {st_loss.avg:>6.4f} '
                    'AE Loss: {ae_loss.avg:>6.4f} '
                    'STAE Loss: {stae_loss.avg:>6.4f} '
                    'LR: {lr:.3e} '
                    'Time: {rate_avg:>7.2f}/s '
                    'Data: {data_time.avg:.3f}'.format(
                        step + 1, num_training_steps,
                        loss=losses_m,
                        st_loss=stae_losses_m,
                        ae_loss=ae_losses_m,
                        stae_loss=stae_losses_m,
                        lr=optimizer.param_groups[0]['lr'],
                        rate_avg=images.size(0) / batch_time_m.avg,
                        data_time=data_time_m)
                )

            if ((step+1) % eval_interval == 0 and step != 0) or (step+1) == num_training_steps:
                model.eval()
                model.quantiles.update(model.map_norm_quantiles(validloader))
