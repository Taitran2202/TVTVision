...
class MultiScaleFusion(nn.Module):
    def __init__(self, channels: Sequence[int] = (64, 128, 256)):
        ...
        self.l1_to_l2 = nn.Conv2d(
            channels[0],  channels[1], stride=2, kernel_size=3, padding=1, groups=channels[0])
        self.l3_to_l2 = UpsampleConv(channels[2], channels[1], scale_factor=2)

        self.l1_to_l3 = nn.Conv2d(
            channels[0], channels[2], stride=4, kernel_size=5, padding=2, groups=channels[0])
        self.l2_to_l3 = nn.Conv2d(
            channels[1], channels[2], stride=2, kernel_size=3, padding=1, groups=channels[1])
