...
def _pa(kernels, emb, label, cc, kernel_num, label_num, min_area=0):
    ...
    inds = np.zeros(
        (label_num, label.shape[0], label.shape[1]), dtype=np.uint8)
        ...
        for j in range(1, i):
            ...
            if rate < 1 / max_rate or rate > max_rate:
                ...
                if flag[j] == 0:
                    flag[j] = 1
                    mean_emb[j] = np.mean(
                        emb[:, inds[j].astype(np.bool_)], axis=1)
