            ...
            for gt_box, gt_label in zip(tgt_box, tgt_cls):
                ...
                if iou_mask.sum() == 0:
                    ...
                    label_assignment_results.append(
                        [grid_x, grid_y, level, anchor_idx])
                else:
                ...
                for result in label_assignment_results:
                    ...
                    if grid_x < fmp_w and grid_y < fmp_h:
                        ...
                        gt_objectness[level][batch_index,
                                             grid_y, grid_x, anchor_idx] = 1.0
                        # cls
                        cls_ont_hot = torch.zeros(self.num_classes)
                        cls_ont_hot[int(gt_label)] = 1.0
                        gt_classes[level][batch_index, grid_y,
                                          grid_x, anchor_idx] = cls_ont_hot
                        # box
                        gt_bboxes[level][batch_index, grid_y, grid_x,
                                         anchor_idx] = torch.as_tensor([x1, y1, x2, y2])
