...
def training(model, trainloader, normalloader, validloader, criterion, optimizer, scheduler, num_training_steps: int = 1000, loss_weights: List[float] = [0.6, 0.4],
    ...
    # criterion
    normal_weighting, boundary_criterion, normal_criterion = criterion
    ...
    for _, (inputs, targets) in enumerate(tqdm(normalloader)):
    ...
    while train_mode:
        ...
        for inputs, targets in trainloader:
            inputs, masks, labels = inputs.to(device, dtype=torch.float32), targets['mask'].to(
                device, dtype=torch.float32), targets['label'].to(device)
            ...
            for layer_idx, layer in enumerate(model.pool_layers):
                encoder_activations = activation[layer].detach()  # BxCxHxW
                batch_size, dim_feature_vector, im_height, im_width = encoder_activations.size()
                mask_ = F.interpolate(masks.unsqueeze(1), size=(
                    im_height, im_width), mode='nearest').squeeze(1)
                ...
                for batch_num in range(fiber_batches):  # per-fiber processing
                    ...
                    decoder_log_prob = get_logp(
                        dim_feature_vector, p_u, log_jac_det)
                    log_prob = decoder_log_prob / dim_feature_vector  # likelihood per dim
                    nor_weights = normal_weighting(log_prob.detach())
                    loss_ml = (-F.logsigmoid(log_prob) * nor_weights).mean()
                    boundaries = boundary_criterion(log_prob, m_b)
                    loss_n_con = normal_criterion(
                        log_prob, m_b, boundaries, 10, weights=nor_weights)
