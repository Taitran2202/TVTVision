...
def visualize_augmentations(dataset, samples=2):
    ...
    for j, (image, text, ignored_map, _, _) in enumerate(loader):
        if j == samples:
            break
        plot_image(image[0].permute(1, 2, 0),
                   text[0].squeeze(), ignored_map[0].squeeze())
...
def run(cfg):
    ...
    # loss
    dclnet_criterion = DCLNetLoss()
    loss_name = "DCLNet Loss"
