...
class Yolov8Bottleneck(nn.Module):
    def __init__(self,
        ...
        self.cv1 = Conv(in_dim, inter_dim, k=kernel_sizes[0], p=kernel_sizes[0] //
                        2, norm_type=norm_type, act_type=act_type, depthwise=depthwise)
        self.cv2 = Conv(inter_dim, out_dim, k=kernel_sizes[1], p=kernel_sizes[1] //
                        2, norm_type=norm_type, act_type=act_type, depthwise=depthwise)
...
class Yolov8StageBlock(nn.Module):
    def __init__(self,
        ...
        self.input_proj = Conv(in_dim, out_dim, k=1,
                               act_type=act_type, norm_type=norm_type)
        self.m = nn.Sequential(*(
            Yolov8Bottleneck(self.inter_dim, self.inter_dim, 1.0, [
                             3, 3], shortcut, act_type, norm_type, depthwise)
            for _ in range(num_blocks)))
        self.output_proj = Conv((2 + num_blocks) * self.inter_dim,
