...
class AugTransforms(object):
    def __init__(self, resize: Tuple[int, int] = (256, 256), use_mask: bool = True,
    ...
    def __call__(self, image, target):
        ...
        if self.anomaly_switch:
            ...
            target['label'] = 1
            self.anomaly_switch = False
            # ignore_mask = np.zeros((1, *self.resize), dtype=np.float32)
        else:
            self.anomaly_switch = True
            # ignore_mask = np.ones((1, *self.resize), dtype=np.float32)
