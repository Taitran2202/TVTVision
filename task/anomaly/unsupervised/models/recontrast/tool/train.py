...
def training(model, trainloader, validloader, criterion, optimizer, scheduler, num_training_steps: int = 1000, loss_weights: List[float] = [0.6, 0.4],
    ...
    # criterion
    cosine_criterion = criterion
    cosine_weight = loss_weights
    ...
    while train_mode:
        ...
        for inputs, targets in trainloader:
            ...
            # predict
            en_features, de_features = model(inputs)
            loss = cosine_weight * cosine_criterion(en_features[:3], de_features[:3], alpha=1, factor=0.) / 2 + \
                cosine_weight * \
                cosine_criterion(
                    en_features[3:], de_features[3:], alpha=1, factor=0.) / 2
