...
def beam_search_decode(emission_log_prob, blank=0, **kwargs):
    ...
    emission_threshold = kwargs.get('emission_threshold', np.log(0.01))
    ...
    for prefix, accu_log_prob in beams:
        ...
        total_accu_log_prob[labels] = \
            logsumexp([accu_log_prob, total_accu_log_prob.get(
                labels, -1 * float('inf'))])
...
def prefix_beam_decode(emission_log_prob, blank=0, **kwargs):
    ...
    beams = [(tuple(), (0, -1 * float('inf')))]

    for t in range(length):
        new_beams_dict = defaultdict(
            lambda: (-1 * float('inf'), -1 * float('inf')))
