...
class ELANBlock(nn.Module):
    def __init__(self, in_dim, out_dim, expand_ratio=0.5, depth=1.0, act_type='silu', norm_type='BN', depthwise=False):
        ...
        for idx in range(round(3*depth)):
            if idx == 0:
                cv3 = [Conv(inter_dim, inter_dim2, k=3, p=1, act_type=act_type,
        ...
        self.cv4 = nn.Sequential(*[
            Conv(inter_dim2, inter_dim2, k=3, p=1, act_type=act_type,
                 norm_type=norm_type, depthwise=depthwise)
            for _ in range(round(3*depth))
        ])
        # output
        self.out = Conv(inter_dim*2 + inter_dim2*2, out_dim,
                        k=1, act_type=act_type, norm_type=norm_type)
...
class ELANBlockFPN(nn.Module):
    def __init__(self, in_dim, out_dim, expand_ratio: float = 0.5, branch_depth: int = 1, shortcut=False, act_type='silu', norm_type='BN', depthwise=False):
        ...
        for i in range(branch_depth):
            if i == 0:
                self.cv3.append(Conv(self.inter_dim1, self.inter_dim2, k=3, p=1,
                                act_type=act_type, norm_type=norm_type, depthwise=depthwise))
            else:
                self.cv3.append(Conv(self.inter_dim2, self.inter_dim2, k=3, p=1,
                                act_type=act_type, norm_type=norm_type, depthwise=depthwise))
        self.cv3 = nn.Sequential(*self.cv3)
        # branch-4
        self.cv4 = nn.Sequential(*[
            Conv(self.inter_dim2, self.inter_dim2, k=3, p=1,
                 act_type=act_type, norm_type=norm_type, depthwise=depthwise)
