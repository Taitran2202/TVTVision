...
class PartialConv(nn.Module):
    def __init__(self, in_dim, out_dim, split_ratio=0.25, kernel_size=1, stride=1, act_type=None, norm_type=None):
        ...
        # ----------- Network Parameters -----------
        self.partial_conv = Conv(self.split_dim, self.split_dim, self.kernel_size,
                                 self.padding, self.stride, act_type=act_type, norm_type=norm_type)
...
class InverseBottleneck(nn.Module):
    def __init__(self,
        ...
        # ----------- Network Parameters -----------
        self.cv1 = Conv(in_dim, in_dim, k=3, p=1, act_type=None,
                        norm_type=norm_type, depthwise=depthwise)
        self.cv2 = Conv(in_dim, self.expand_dim, k=1, act_type=act_type,
                        norm_type=norm_type, depthwise=depthwise)
        self.cv3 = Conv(self.expand_dim, out_dim, k=1, act_type=act_type,
                        norm_type=norm_type, depthwise=depthwise)
...
class ELANBlockFPN(nn.Module):
    def __init__(self, in_dim, out_dim, expand_ratio: float = 0.5, branch_depth: int = 1, shortcut=False, act_type='silu', norm_type='BN', depthwise=False):
        ...
        # ----------- Network Parameters -----------
        # branch-1
        self.cv1 = Conv(in_dim, self.inter_dim1, k=1,
                        act_type=act_type, norm_type=norm_type)
        # branch-2
        self.cv2 = Conv(in_dim, self.inter_dim1, k=1,
                        act_type=act_type, norm_type=norm_type)
        # branch-3
        self.cv3 = []
        for i in range(branch_depth):
            if i == 0:
                self.cv3.append(YoloBottleneck(self.inter_dim1, self.inter_dim2, [
                                1, 3], 1.0, shortcut, act_type, norm_type, depthwise))
            else:
                self.cv3.append(YoloBottleneck(self.inter_dim2, self.inter_dim2, [
                                1, 3], 1.0, shortcut, act_type, norm_type, depthwise))
        self.cv3 = nn.Sequential(*self.cv3)
        # branch-4
        self.cv4 = nn.Sequential(*[
            YoloBottleneck(self.inter_dim2, self.inter_dim2, [
                           1, 3], 1.0, shortcut, act_type, norm_type, depthwise)
            for _ in range(branch_depth)
        ])
        # branch-5
        ...
        # output proj
        self.out = Conv(self.inter_dim1*2 + self.inter_dim2*4,
                        out_dim, k=1, act_type=act_type, norm_type=norm_type)
...
def build_downsample_layer(cfg, in_dim, out_dim):
    if cfg['fpn_downsample_layer'] == 'conv':
        ...
        layer = DSBlock(in_dim, out_dim,
                        cfg['fpn_act'], cfg['fpn_norm'], cfg['fpn_depthwise'])
