...
from ....utils.box_ops import bbox_iou, bbox2dist
from .matcher import TaskAlignedAssigner


class YOLOLoss(object):
    def __init__(self, reg_max, loss_cls_weight, loss_box_weight, loss_dfl_weight, topk_candidates, alpha, beta, num_classes=80):
        # --------------- Basic parameters ---------------
    ...
    def loss_classes(self, pred_cls, gt_score):
        ...
        loss_cls = F.binary_cross_entropy_with_logits(
            pred_cls, gt_score, reduction='none')
    ...
    def __call__(self, outputs, targets, epoch=0):
        ...
        loss_box = self.loss_bboxes(
            box_preds_pos, box_targets_pos, bbox_weight)
        ...
        loss_dfl = self.loss_dfl(
            reg_preds_pos, box_targets_pos, anchors_pos, strides_pos, bbox_weight)
        loss_dfl = loss_dfl.sum() / num_fgs

        # total loss
        losses = loss_cls * self.loss_cls_weight + loss_box * \
            self.loss_box_weight + loss_dfl * self.loss_dfl_weight
        loss_dict = dict(
            loss_cls=loss_cls,
            loss_box=loss_box,
            loss_obj=loss_dfl,
