...
def load_model(cfg, model_name, target):
    ...
    model = PatchcoreRS(
        ...
        memory_bank_size=checkpoint['memory_bank_size']
    ).to(device)
...
def result_plot(idx, model, testset, params, test_img):
    ...
    mask = output_i[0].cpu().detach().squeeze()
    heat_map = compute_anomaly_mask(mask, threshold_pixel)
    anomaly_score = output_i[1].cpu().detach().squeeze()
    prediction = anomaly_score.numpy() > threshold_image
