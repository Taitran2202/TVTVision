...
class EmbLoss(nn.Module):
    ...
    def forward_single(self, emb, instance, kernel, training_mask, bboxes):
        ...
        emb_mean = emb.new_zeros(
            (self.feature_dim, num_instance), dtype=torch.float32)
        ...
        if num_instance > 2:
            ...
            emb_band = emb_mean.permute(1, 0).repeat(
                1, num_instance).view(-1, self.feature_dim)
            # print(seg_band)

            mask = (1 - torch.eye(num_instance, dtype=torch.int8)
                    ).view(-1, 1).repeat(1, self.feature_dim)
    ...
    def forward(self, emb, instance, kernel, training_mask, bboxes, reduce=True):
        ...
        for i in range(loss_batch.size(0)):
            loss_batch[i] = self.forward_single(
                emb[i], instance[i], kernel[i], training_mask[i], bboxes[i])
