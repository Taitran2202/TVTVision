...
def training(model, trainloader, validloader, criterion, optimizer, scheduler, num_training_steps: int = 1000, loss_weights: List[float] = [0.6, 0.4],
    ...
    # criterion
    dsr_second_stage_criterion, dsr_thrid_stage_criterion = criterion
    ...
    while train_mode:
        ...
        for inputs, targets in trainloader:
            ...
            if (step+1) <= num_training_steps * 0.7:
                for parameters in model.upsampling_module.parameters():
                    parameters.requires_grad = False

                # predict
                y_preds = model(inputs, masks.unsqueeze(1))
                loss = dsr_second_stage_criterion(
                    y_preds["recon_feat_hi"],
                    y_preds["recon_feat_lo"],
                    y_preds["embedding_bot"],
                    y_preds["embedding_top"],
                    inputs,
                    y_preds["obj_spec_image"],
                    y_preds["anomaly_map"],
                    y_preds["true_anomaly_map"],
                )
            else:
                ...
                # predict
                y_preds = model(augmented_inputs)
                loss = dsr_thrid_stage_criterion(
                    y_preds["anomaly_map"], masks.unsqueeze(1))
