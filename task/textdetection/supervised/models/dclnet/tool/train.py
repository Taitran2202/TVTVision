...
from utils.post_processing.textdetection.dclnet import test_net
...
def train(model, dataloader, criterion, optimizer, loss_weights: List[float], log_interval: int, device: str) -> dict:
    ...
    # criterion
    dclnet_criterion = criterion
    dclnet_weight = loss_weights
    ...
    for idx, (images, text, ignore, rho, theta) in enumerate(dataloader):
        images, text, ignore, rho, theta = list(
            map(lambda x: x.to(device), [images, text, ignore, rho, theta]))

        data_time_m.update(time.time() - end)

        # predict
        pred_cls, pred_rho, pred_theta = model(images)
        dclnet_loss = dclnet_criterion(
            text, ignore, rho, theta, pred_cls, pred_rho, pred_theta)

        # detection loss
        loss = dclnet_weight * dclnet_loss
        loss.backward()
...
def test(model, dataset, criterion, cls_thresh: float, nms_thresh: float,
         resize: int, vis_test_dir: str, log_interval: int, device: str) -> dict:
    # metrics
...
def fit(
    ...
    loss_weights: List[float], cls_thresh: float, nms_thresh: float,
    resize: int, epochs: int, savedir: str, log_interval: int,
    eval_interval: int, resume: bool, save_model_path: str,
