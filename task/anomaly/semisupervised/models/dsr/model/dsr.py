...
class DSR(nn.Module):
    ...
    def __init__(
        ...
        self.subspace_restriction_module_lo = SubspaceRestrictionModule(
            base_width=embedding_dim)
        self.subspace_restriction_module_hi = SubspaceRestrictionModule(
            base_width=embedding_dim)
        ...
        if anomaly_map_to_generate is None:
            ...
            with torch.no_grad():
                ...
                _, recon_embd_bot = self.subspace_restriction_module_hi(
                    anomaly_embedding_bot_copy, embedder_bot)
                _, recon_embd_top = self.subspace_restriction_module_lo(
                    anomaly_embedding_top_copy, embedder_top)

                # Upscale top (lo) embedding
                up_quantized_recon_t = self.discrete_latent_model.upsample_t(
                    recon_embd_top)

                # Concat embeddings and reconstruct image (object specific decoder)
                quant_join = torch.cat(
                    (up_quantized_recon_t, recon_embd_bot), dim=1)
                obj_spec_image = self.image_reconstruction_network(quant_join)

                # Anomaly detection module
                out_mask = self.anomaly_detection_module(
                    obj_spec_image, gen_image)
                out_mask_sm = torch.softmax(out_mask, dim=1)

            # Mask upsampling and score calculation
            upsampled_mask = self.upsampling_module(
                obj_spec_image, gen_image, out_mask_sm)
            ...
            anom_str_lo = (
                torch.rand(
                    batch.shape[0]) * (1.0 - self.latent_anomaly_strength) + self.latent_anomaly_strength
            ).cuda()
            anom_str_hi = (
                torch.rand(
                    batch.shape[0]) * (1.0 - self.latent_anomaly_strength) + self.latent_anomaly_strength
            ...
            raise RuntimeError(
                "There should not be an anomaly map to generate when not training")
...
class SubspaceRestrictionModule(nn.Module):
    ...
    def __init__(self, base_width: int):
        ...
        self.unet = SubspaceRestrictionNetwork(
            in_channels=base_width, out_channels=base_width, base_width=base_width)
...
class FeatureEncoder(nn.Module):
    ...
    def __init__(self, in_channels: int, base_width: int):
        ...
        self.block2 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width * 2, base_width *
        ...
        self.block3 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width * 4, base_width *
                      4, kernel_size=3, padding=1),
            nn.InstanceNorm2d(base_width * 4),
            nn.ReLU(inplace=True),
        )
...
class ImageReconstructionNetwork(nn.Module):
    ...
    def __init__(self, in_channels: int, num_hiddens: int, num_residual_layers: int, num_residual_hiddens: int):
        ...
        self.pre_vq_conv = nn.Conv2d(
            in_channels=in_channels * 4, out_channels=64, kernel_size=1, stride=1)
        ...
        self._conv_1 = nn.Conv2d(
            in_channels=64, out_channels=num_hiddens, kernel_size=3, stride=1, padding=1)
...
class UnetDecoder(nn.Module):
    ...
    def __init__(self, base_width: int, out_channels: int = 1):
        ...
        self.db1 = nn.Sequential(
            nn.Conv2d(base_width * (4 + 4), base_width *
                      4, kernel_size=3, padding=1),
            norm_layer(base_width * 4),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width * 4, base_width *
        ...
        # cat with base*2
        self.db2 = nn.Sequential(
            nn.Conv2d(base_width * (2 + 2), base_width *
                      2, kernel_size=3, padding=1),
            norm_layer(base_width * 2),
            nn.ReLU(inplace=True),
            nn.Conv2d(base_width * 2, base_width *
        ...
        # cat with base*1
        self.db3 = nn.Sequential(
            nn.Conv2d(base_width * (1 + 1), base_width,
...
class UpsamplingModule(nn.Module):
    ...
    def forward(self, batch_real: Tensor, batch_anomaly: Tensor, batch_segmentation_map: Tensor) -> Tensor:
        ...
        """
        img_x = torch.cat((batch_real, batch_anomaly,
                          batch_segmentation_map), dim=1)
        x = self.unet(img_x)
        return x
...
class EncoderTop(nn.Module):
    ...
    def __init__(self, in_channels: int, num_hiddens: int, num_residual_layers: int, num_residual_hiddens: int):
        ...
        self._conv_1 = nn.Conv2d(
            in_channels=in_channels, out_channels=num_hiddens, kernel_size=4, stride=2, padding=1)
        self._conv_2 = nn.Conv2d(
            in_channels=num_hiddens, out_channels=num_hiddens, kernel_size=3, stride=1, padding=1)
        self._residual_stack = ResidualStack(
            in_channels=num_hiddens,
...
class DecoderBot(nn.Module):
    ...
    def __init__(self, in_channels: int, num_hiddens: int, num_residual_layers: int, num_residual_hiddens: int):
        ...
        self._conv_1 = nn.Conv2d(
            in_channels=in_channels, out_channels=num_hiddens, kernel_size=3, stride=1, padding=1)
...
class DiscreteLatentModel(nn.Module):
    ...
    def __init__(
        ...
        self._pre_vq_conv_top = nn.Conv2d(
        ...
        self._decoder_b = DecoderBot(
            embedding_dim * 2, num_hiddens, num_residual_layers, num_residual_hiddens)
    ...
    def generate_fake_anomalies_joined(
        ...
        random_embeddings = torch.zeros(
            (embeddings.shape[0], embeddings.shape[2] *
             embeddings.shape[3], memory_torch_original.shape[1])
        )
        ...
        for k in range(embeddings.shape[0]):
            ...
            topk = max(1, min(int(percentage_vectors *
                       memory_torch.shape[0]) + 1, memory_torch.shape[0] - 1))
            _, topk_indices = torch.topk(
                distances_b, topk, dim=1, largest=False)
            topk_indices = topk_indices[:, int(memory_torch.shape[0] * 0.05):]
            topk = topk_indices.shape[1]

            random_indices_hik = torch.randint(
                topk, size=(topk_indices.shape[0],))
            random_indices_t = topk_indices[torch.arange(
                random_indices_hik.shape[0]), random_indices_hik]
            random_embeddings[k] = memory_torch[random_indices_t, :]
        random_embeddings = random_embeddings.reshape(
            (random_embeddings.shape[0], embeddings.shape[2],
             embeddings.shape[3], random_embeddings.shape[2])
        )
        ...
        anomaly_mask = torch.nn.functional.max_pool2d(
            mask, (down_ratio_y, down_ratio_x)).float()

        anomaly_embedding = anomaly_mask * random_embeddings_tensor + \
            (1.0 - anomaly_mask) * embeddings
    ...
    def forward(
        ...
        if anomaly_mask is not None:
            ...
            use_both = torch.randint(
                0, 2, (batch.shape[0], 1, 1, 1)).cuda().float()
            ...
            anomaly_embedding_hi_not_both = use_hi * \
                anomaly_embedding_hi_usebot + use_lo * anomaly_embedding_hi_usetop
            anomaly_embedding_lo_not_both = use_hi * \
                anomaly_embedding_lo_usebot + use_lo * anomaly_embedding_lo_usetop
            anomaly_embedding_hi = (
                (anomaly_embedding_hi * use_both +
                 anomaly_embedding_hi_not_both * (1.0 - use_both)).detach().clone()
            )
            anomaly_embedding_lo = (
                (anomaly_embedding_lo * use_both +
                 anomaly_embedding_lo_not_both * (1.0 - use_both)).detach().clone()
            )
            ...
            quant_join_anomaly = torch.cat(
                (up_quantized_anomaly_t, anomaly_embedding_hi_copy), dim=1)
            ...
            down_ratio_x_hi = int(
                anomaly_mask.shape[3] / quantized_b_defect.shape[3])
            anomaly_mask_hi = torch.nn.functional.max_pool2d(
                anomaly_mask, (down_ratio_x_hi, down_ratio_x_hi)).float()
            anomaly_mask_hi = torch.nn.functional.interpolate(
                anomaly_mask_hi, scale_factor=down_ratio_x_hi)
            down_ratio_x_lo = int(anomaly_mask.shape[3] / quantized_t.shape[3])
            anomaly_mask_lo = torch.nn.functional.max_pool2d(
                anomaly_mask, (down_ratio_x_lo, down_ratio_x_lo)).float()
            anomaly_mask_lo = torch.nn.functional.interpolate(
                anomaly_mask_lo, scale_factor=down_ratio_x_lo)
