...
class YOLOLoss(object):
    def __init__(self, loss_obj_weight, loss_cls_weight, loss_box_weight, anchor_thresh, anchor_size, device, num_classes=80):
        self.device = device
        ...
        # matcher
        self.matcher = Yolov5Matcher(
            num_classes, 3, anchor_size, anchor_thresh)
    ...
    def __call__(self, outputs, targets, epoch=0):
        ...
        pred_obj = torch.cat(
            outputs['pred_obj'], dim=1).view(-1)                      # [BM,]
        pred_cls = torch.cat(
            outputs['pred_cls'], dim=1).view(-1, self.num_classes)    # [BM, C]
        pred_box = torch.cat(
            outputs['pred_box'], dim=1).view(-1, 4)                   # [BM, 4]
