...
from .module import AnomalyMapGenerator, FeatureExtractor, bgad_head, get_logp, positional_encoding_2d


class BGAD(nn.Module):
    def __init__(
        self,
        ...
        self.encoder = FeatureExtractor(
            backbone=self.backbone, layers=self.pool_layers, pre_trained=pre_trained)
        ...
        self.anomaly_map_generator = AnomalyMapGenerator(
            image_size=input_size, pool_layers=self.pool_layers)
    ...
    def forward(self, images) -> Any:
        ...
        distribution = [torch.Tensor(0).to(images.device)
                        for _ in self.pool_layers]
        ...
        for layer_idx, layer in enumerate(self.pool_layers):
            ...
            pos_encoding = einops.repeat(
                positional_encoding_2d(
                    self.condition_vector, im_height, im_width).unsqueeze(0),
                "b c h w-> (tile b) c h w",
                tile=batch_size,
            ).to(images.device)
            c_r = einops.rearrange(
                pos_encoding, "b c h w -> (b h w) c")  # BHWxP
            e_r = einops.rearrange(encoder_activations,
                                   "b c h w -> (b h w) c")  # BHWxC
            ...
            for batch_num in range(fiber_batches):  # per-fiber processing
                if batch_num < (fiber_batches - 1):
                    idx = torch.arange(
                        batch_num * self.fiber_batch_size, (batch_num + 1) * self.fiber_batch_size)
                ...
                distribution[layer_idx] = torch.cat(
                    (distribution[layer_idx], log_prob))
