...
class ELANNet_Huge(nn.Module):
    def __init__(self, depthwise=False, num_classes=1000):
        ...
        self.layer_1 = nn.Sequential(
            ...
            # P1/2
            Conv(80, 80, k=3, p=1, act_type='silu', depthwise=depthwise)
        )
        self.layer_2 = nn.Sequential(
...
class ELANNet_Nano(nn.Module):
    def __init__(self, depthwise=True, num_classes=1000):
        ...
        # tiny backbone
        self.layer_1 = Conv(3, 16, k=3, p=1, s=2,
                            act_type='lrelu', depthwise=depthwise)       # P1/2
...
# build elannet
def build_elannet(model_name='large', pretrained=False, num_classes=None):
    # model
    if model_name == 'large':
        model = ELANNet_Large()
    ...
    url = model_urls['elannet_' + model_name]
    # load weight
