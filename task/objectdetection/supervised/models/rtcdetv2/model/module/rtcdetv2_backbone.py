...
from .rtcdetv2_basic import Conv, ELANBlock, DSBlock
...
class ELANNet(nn.Module):
    def __init__(self, width=1.0, depth=1.0, act_type='silu', norm_type='BN', depthwise=False):
        ...
        self.feat_dims = [round(64*width), round(128*width), round(256*width),
                          round(512*width), round(1024*width), round(1024*width)]

        # ------------------ Network parameters ------------------
        # P1/2
        self.layer_1 = nn.Sequential(
            Conv(3, self.feat_dims[0], k=3, p=1, s=2,
                 act_type=act_type, norm_type=norm_type),
            Conv(self.feat_dims[0], self.feat_dims[0], k=3, p=1,
                 act_type=act_type, norm_type=norm_type, depthwise=depthwise)
        )
        # P2/4
        self.layer_2 = nn.Sequential(
            Conv(self.feat_dims[0], self.feat_dims[1], k=3, p=1, s=2,
                 act_type=act_type, norm_type=norm_type, depthwise=depthwise),
            ELANBlock(self.feat_dims[1], self.feat_dims[2],
                      self.expand_ratios[0], self.depth, act_type, norm_type, depthwise)
        )
        # P3/8
        self.layer_3 = nn.Sequential(
            DSBlock(self.feat_dims[2], self.feat_dims[2],
                    act_type, norm_type, depthwise),
            ELANBlock(self.feat_dims[2], self.feat_dims[3],
                      self.expand_ratios[1], self.depth, act_type, norm_type, depthwise)
        )
        # P4/16
        self.layer_4 = nn.Sequential(
            DSBlock(self.feat_dims[3], self.feat_dims[3],
                    act_type, norm_type, depthwise),
            ELANBlock(self.feat_dims[3], self.feat_dims[4],
                      self.expand_ratios[2], self.depth, act_type, norm_type, depthwise)
        )
        # P5/32
        self.layer_5 = nn.Sequential(
            DSBlock(self.feat_dims[4], self.feat_dims[4],
                    act_type, norm_type, depthwise),
            ELANBlock(self.feat_dims[4], self.feat_dims[5],
                      self.expand_ratios[3], self.depth, act_type, norm_type, depthwise)
        )
