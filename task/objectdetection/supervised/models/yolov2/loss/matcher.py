...
class Yolov2Matcher(object):
    ...
    def __call__(self, fmp_size, stride, targets):
        ...
        gt_classes = np.zeros(
            [bs, fmp_h, fmp_w, self.num_anchors, self.num_classes])
            ...
            for gt_box, gt_label in zip(tgt_box, tgt_cls):
                ...
                for result in label_assignment_results:
                    ...
                    if grid_x < fmp_w and grid_y < fmp_h:
                        # obj
                        gt_objectness[batch_index, grid_y,
                                      grid_x, anchor_idx] = 1.0
                        # cls
                        cls_ont_hot = np.zeros(self.num_classes)
                        cls_ont_hot[int(gt_label)] = 1.0
                        gt_classes[batch_index, grid_y,
                                   grid_x, anchor_idx] = cls_ont_hot
                        # box
                        gt_bboxes[batch_index, grid_y, grid_x,
                                  anchor_idx] = np.array([x1, y1, x2, y2])
