...
class MSGMS_Loss(nn.Module):
    def __init__(self, num_scales: int = 4, in_channels: int = 3) -> None:
        super(MSGMS_Loss, self).__init__()
    ...
    def forward(self, img1: Tensor, img2: Tensor, as_loss: bool = True) -> Tensor:
        ...
        for scale in range(self.num_scales):
            ...
            msgms_map += F.interpolate(gms_map, size=(h, w),
    ...
    def _gms(self, img1: Tensor, img2: Tensor) -> Tensor:
        ...
        gm1_x = F.conv2d(img1, self.prewitt_x, stride=1,
                         padding=1, groups=self.in_channels)
        gm1_y = F.conv2d(img1, self.prewitt_y, stride=1,
                         padding=1, groups=self.in_channels)
        gm1 = torch.sqrt(gm1_x ** 2 + gm1_y ** 2 + 1e-12)

        gm2_x = F.conv2d(img2, self.prewitt_x, stride=1,
                         padding=1, groups=self.in_channels)
        gm2_y = F.conv2d(img2, self.prewitt_y, stride=1,
                         padding=1, groups=self.in_channels)
