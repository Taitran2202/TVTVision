...
class Conv(nn.Module):
    def __init__(self,
        ...
        if depthwise:
            convs.append(get_conv2d(c1, c1, k=k, p=p, s=s, d=d,
                         g=c1, padding_mode=padding_mode, bias=add_bias))
            # depthwise conv
            if norm_type:
            ...
            convs.append(get_conv2d(c1, c2, k=k, p=p, s=s, d=d,
                         g=1, padding_mode=padding_mode, bias=add_bias))
            if norm_type:
                convs.append(get_norm(norm_type, c2))
            if act_type:
                convs.append(get_activation(act_type))
