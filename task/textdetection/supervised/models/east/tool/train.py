...
from utils.post_processing.textdetection.east import test_net
...
def train(model, dataloader, criterion, optimizer, loss_weights: List[float], log_interval: int, device: str) -> dict:
    ...
    for idx, (images, gt_scores, gt_geos, ignored_maps) in enumerate(dataloader):
        images, gt_scores, gt_geos, ignored_maps = images.to(device), gt_scores.to(
            device), gt_geos.to(device), ignored_maps.to(device)
