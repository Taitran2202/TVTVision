...
class DCLNetLoss(nn.Module):
    def __init__(self):
        super(DCLNetLoss, self).__init__()
    ...
    def forward(self, text, ignore, rho, theta, pred_cls, pred_rho, pred_theta):
        ...
        rho_map = -torch.log(torch.min(pred_rho, rho) /
                             (torch.max(pred_rho, rho) + 1e-8) + 1e-8)
        rho_loss = torch.sum(rho_map * mask) / (torch.sum(mask) + 1e-8)

        theta_loss = torch.sum(
            torch.sin(torch.abs(pred_theta - theta) * mask / 2)) / (torch.sum(mask) + 1e-8)
