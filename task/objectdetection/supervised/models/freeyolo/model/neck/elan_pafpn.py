...
class ELANBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, size='large', depthwise=False, act_type='silu', norm_type='BN'):
        ...
        for idx in range(width):
            if idx == 0:
                cvs = [Conv(inter_dim, inter_dim2, k=3, p=1, act_type=act_type,
                            norm_type=norm_type, depthwise=depthwise)]
            else:
                cvs = [Conv(inter_dim2, inter_dim2, k=3, p=1, act_type=act_type,
                            norm_type=norm_type, depthwise=depthwise)]
            # deeper
            if depth > 1:
        ...
        self.out = Conv(inter_dim*2+inter_dim2*len(self.cv3),
...
class ELAN_PaFPN(nn.Module):
    def __init__(self,
        ...
        # top dwon
        # P5 -> P4
        self.cv1 = Conv(c5, int(256 * width), k=1,
                        norm_type=norm_type, act_type=act_type)
        self.cv2 = Conv(c4, int(256 * width), k=1,
                        norm_type=norm_type, act_type=act_type)
        ...
        # P4 -> P3
        self.cv3 = Conv(int(256 * width), int(128 * width), k=1,
                        norm_type=norm_type, act_type=act_type)
        self.cv4 = Conv(c3, int(128 * width), k=1,
                        norm_type=norm_type, act_type=act_type)
        ...
        if fpn_size == 'large' or fpn_size == 'huge':
            ...
            self.mp1 = nn.Sequential(
                nn.MaxPool2d((2, 2), 2),
                Conv(int(128 * width), int(256 * width), k=1,
                     act_type=act_type, norm_type=norm_type)
            )
        ...
        if fpn_size == 'large' or fpn_size == 'huge':
            ...
            self.mp2 = nn.Sequential(
                nn.MaxPool2d((2, 2), 2),
                Conv(int(256 * width), int(512 * width), k=1,
                     act_type=act_type, norm_type=norm_type)
            )
        ...
        if self.out_dim is not None:
            ...
            self.out_dim = [
                int(256 * width), int(512 * width), int(1024 * width)]
