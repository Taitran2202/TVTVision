...
def compute_new_image_size(image_size: tuple, tile_size: tuple, stride: tuple) -> tuple:
    ...
    def __compute_new_edge_size(edge_size: int, tile_size: int, stride: int) -> int:
        ...
        if (edge_size - tile_size) % stride != 0:
            edge_size = (math.ceil((edge_size - tile_size) /
                         stride) * stride) + tile_size
...
class Tiler:
    ...
    def __init__(
        ...
        self.tile_size_h, self.tile_size_w = self.__validate_size_type(
            tile_size)
    ...
    def __random_tile(self, image: Tensor) -> Tensor:
        ...
        """
        return torch.vstack([transforms.RandomCrop(self.tile_size_h)(image) for i in range(self.tile_count)])
    ...
    def __unfold(self, tensor: Tensor) -> Tensor:
        ...
        self.num_patches_h = int(
            (image_h - self.tile_size_h) / self.stride_h) + 1
        self.num_patches_w = int(
            (image_w - self.tile_size_w) / self.stride_w) + 1

        # create an empty torch tensor for output
        tiles = torch.zeros(
            (self.num_patches_h, self.num_patches_w, batch, channels,
             self.tile_size_h, self.tile_size_w), device=device
        )
    ...
    def __fold(self, tiles: Tensor) -> Tensor:
        ...
        scale_h, scale_w = (
            tile_size_h / self.tile_size_h), (tile_size_w / self.tile_size_w)
        # identify device type based on input tensor
        device = tiles.device
        ...
        image_size = (self.batch_size, num_channels, int(
            self.resized_h * scale_h), int(self.resized_w * scale_w))
        ...
        for patch, (loc_i, loc_j) in zip(
            ...
            img[:, :, loc_i: (loc_i + reduced_tile_h),
                loc_j: (loc_j + reduced_tile_w)] += patch
            lookup[:, :, loc_i: (loc_i + reduced_tile_h),
                   loc_j: (loc_j + reduced_tile_w)] += ones
    ...
    def tile(self, image: Tensor, use_random_tiling: bool | None = False) -> Tensor:
        ...
        image = upscale_image(image, size=(
            self.resized_h, self.resized_w), mode=self.mode)
    ...
    def untile(self, tiles: Tensor) -> Tensor:
        ...
        image = downscale_image(image=image, size=(
            self.input_h, self.input_w), mode=self.mode)
