...
def training(model, trainloader, validloader, criterion, optimizer, scheduler, num_training_steps: int = 1000, loss_weights: List[float] = [0.6, 0.4],
    ...
    # criterion
    l2_criterion, l1_criterion, focal_criterion = criterion
    l2_weight, l1_weight, focal_weight = loss_weights
    ...
    while train_mode:
        ...
        for inputs, targets in trainloader:
            ...
            # predict
            noise, noise_pred, y_preds = model(inputs)
            # noise *= targets['ignore_mask'].to(device)
            # noise_pred *= targets['ignore_mask'].to(device)
            l2_loss = l2_criterion(noise_pred, noise)
            l1_loss = l1_criterion(y_preds[:, 0, :], 1 - masks)
            focal_loss = focal_criterion(y_preds, masks)
            loss = (l2_weight * l2_loss) + (l1_weight *
                                            l1_loss) + (focal_weight * focal_loss)
            loss.backward()
