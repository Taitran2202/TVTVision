...
class AugTransforms(object):
    def __init__(self, datadir: str, target: str,
                 use_mask: bool = True, resize: Tuple[int, int] = (256, 256),
                 texture_source_dir: str = None, trans_config=None,
    ...
    def rand_augment_for_real(self):
        augmenters = [
            A.RandomRotate90(),
            A.Flip(),
            A.Transpose(),
        ...
        aug = A.Compose([augmenters[aug_ind[0]],
                         augmenters[aug_ind[1]],
                         augmenters[aug_ind[2]]])
        return aug

    def rand_augment_for_pseudo(self):
        augmenters = [
    ...
    def __call__(self, image, target):
        ...
        if self.anomaly_switch and self.file_list and np.random.uniform() < 0.5:
            ...
            transformed = self.rand_augment_for_real()(image=image, mask=mask)
            ...
            transformed = self.rand_augment_for_real()(
                image=image, mask=target['mask'])
            image = transformed["image"]
            target['mask'] = transformed["mask"]
        ...
        if p > 0.7:
            ...
            target['mask'] = rot(image=target['mask'])
    ...
    def _structure_source(self, img: np.ndarray) -> np.ndarray:
        structure_source_img = self.rand_augment_for_pseudo()(image=img)
