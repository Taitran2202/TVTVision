...
def adjust_height(img, vertices, ratio=0.2):
    ...
    Output:
    img : adjusted numpy.ndarray image
    new_vertices: adjusted vertices
    '''
    ratio_h = 1 + ratio * (np.random.rand() * 2 - 1)
    old_h, old_w = img.shape[:2]
    new_h = int(np.around(old_h * ratio_h))
    img = cv2.resize(img, (old_w, new_h), interpolation=cv2.INTER_LINEAR)
...
def rotate_img(img, vertices, angle_range=10):
    ...
    Output:
            img         : rotated numpy array of shape (H, W, C)
            new_vertices: rotated vertices
    '''
    center_x = (img.shape[1] - 1) / 2
    center_y = (img.shape[0] - 1) / 2
    angle = angle_range * (np.random.rand() * 2 - 1)
    img = rotate_image(img, angle, center_x, center_y)
    new_vertices = np.zeros(vertices.shape)
...
def rotate_image(img, angle, center_x, center_y):
    '''rotate image numpy array
    Input:
            img         : numpy array of shape (H, W, C)
    ...
    Output:
            rotated_img : rotated numpy array of shape (H, W, C)
    '''
    H, W, _ = img.shape
...
def get_score_geo(img, vertices, labels, scale, length):
    ...
    '''
    score_map = np.zeros(
        (int(img.shape[0] * scale), int(img.shape[1] * scale), 1), np.float32)
    geo_map = np.zeros(
        (int(img.shape[0] * scale), int(img.shape[1] * scale), 5), np.float32)
