...
from utils.post_processing.textdetection.psenet import test_net
...
def train(model, dataloader, criterion, optimizer, loss_weights: List[float], log_interval: int, device: str) -> dict:
    ...
    # criterion
    pse_criterion = criterion
    text_weight, kernel_weight = loss_weights
    ...
    for idx, data in enumerate(dataloader):
        ...
        det_loss = pse_criterion(
            det_out, gt_texts, gt_kernels, training_masks, text_weight, kernel_weight)
    ...
    return OrderedDict([('loss', losses_m.avg), ('text loss', losses_text_m.avg), ('kernel loss', losses_kernels_m.avg)])


def test(model, dataset, criterion, kernel_num: int, min_area: float,
         min_score: float, bbox_type: str, resize: int, vis_test_dir: str,
...
def fit(
    ...
    loss_weights: List[float], kernel_num: int, min_area: float, min_score: float,
    use_wandb: bool, bbox_type: str, resize: int, epochs: int, savedir: str,
