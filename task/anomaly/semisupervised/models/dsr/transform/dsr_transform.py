...
class AugTransforms(object):
    def __init__(self, use_mask: bool = True, resize: Tuple[int, int] = (256, 256),
    ...
    def __call__(self, image, target):
        if self.anomaly_switch:
            augmented_image, mask = self.generate_anomaly(img=image)
            target['augmented_image'] = augmented_image
            target['mask'] = mask
            target['label'] = 1
            self.anomaly_switch = False
        else:
            target['augmented_image'] = image
        ...
        if p > 0.7:
            ...
            target['augmented_image'] = rot(image=target['augmented_image'])
            target['mask'] = rot(image=target['mask'])

        image = self.img_transform(image)
        target['augmented_image'] = self.img_transform(
            target['augmented_image'])
