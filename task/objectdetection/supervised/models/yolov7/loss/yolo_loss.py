...
class YOLOLoss(object):
    def __init__(self, center_sampling_radius, topk_candicate, loss_obj_weight=1.0, loss_cls_weight=1.0, loss_box_weight=1.0, num_classes=80, max_epoch=None, no_aug_epoch=None):
    ...
    def __call__(self, outputs, targets, epoch=0):
        ...
        if epoch >= (self.max_epoch - self.no_aug_epoch - 1):
            ...
            # anchor tensors
            anchors_tensors = torch.cat(outputs['anchors'], dim=0)[
                None].repeat(bs, 1, 1)
            anchors_tensors_pos = anchors_tensors.view(-1, 2)[fg_masks]
            # stride tensors
            stride_tensors = torch.cat(outputs['stride_tensors'], dim=0)[
                None].repeat(bs, 1, 1)
            stride_tensors_pos = stride_tensors.view(-1, 1)[fg_masks]
            # aux loss
            loss_box_aux = self.loss_bboxes_aux(
                reg_preds_pos, box_targets, anchors_tensors_pos, stride_tensors_pos)
