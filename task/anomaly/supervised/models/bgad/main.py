...
from .model import BGAD
from .tool import training
from .transform import AugTransforms
from .loss import normal_fl_weighting, Boundary, calculate_bg_spp_loss_normal
...
def run(cfg):
    ...
    normalloader = create_dataloader(
        dataset=normalset,
        is_train=False,
        batch_size=cfg.DATALOADER.batch_size,
        num_workers=cfg.DATALOADER.num_workers
    ...
    model = BGAD(
        ...
        fiber_batch_size=cfg.MODEL.fiber_batch_size,
        decoder=cfg.MODEL.decoder,
        condition_vector=cfg.MODEL.condition_vector,
        coupling_blocks=cfg.MODEL.coupling_blocks,
        clamp_alpha=cfg.MODEL.clamp_alpha,
        permute_soft=cfg.MODEL.permute_soft,
    )
    model.to(device)
    ...
    # loss
    normal_weighting = normal_fl_weighting
    boundary_criterion = Boundary(
        pos_beta=cfg.TRAIN.boundary_pos_beta,
        margin_tau=cfg.TRAIN.boundary_margin_tau,
        normalizer=cfg.TRAIN.boundary_normalizer
    )
    normal_criterion = calculate_bg_spp_loss_normal
    ...
    training(
        ...
        criterion=[normal_weighting, boundary_criterion, normal_criterion],
        optimizer=optimizer,
        scheduler=scheduler,
