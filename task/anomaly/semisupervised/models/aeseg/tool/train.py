...
def training(model, trainloader, validloader, criterion, optimizer, scheduler, num_training_steps: int = 1000, loss_weights: List[float] = [0.6, 0.4],
    ...
    l2_losses_m = AverageMeter()
    ssim_losses_m = AverageMeter()
    ...
    # criterion
    l2_criterion, ssim_criterion, l1_criterion, focal_criterion = criterion
    l2_weight, ssim_weight, l1_weight, focal_weight = loss_weights
    ...
    while train_mode:
        ...
        for inputs, targets in trainloader:
            ...
            augmented_inputs = targets['augmented_image'].to(
                device, dtype=torch.float32)
            masks = targets['mask'].to(device, dtype=torch.float32)
            labels = targets['label'].to(device)

            data_time_m.update(time.time() - end)

            # predict
            reconstruction, prediction = model(augmented_inputs)
            l2_loss = l2_criterion(reconstruction, model.normalize(inputs))
            ssim_loss = ssim_criterion(reconstruction, model.normalize(inputs))
            l1_loss = l1_criterion(prediction[:, 0, :], 1 - masks)
            focal_loss = focal_criterion(prediction, masks)
            loss = (l2_weight * l2_loss) + (ssim_weight * ssim_loss) + \
                (l1_weight * l1_loss) + (focal_weight * focal_loss)

            if model.sspcab:
                loss += model.sspcab_lambda * model.sspcab_loss(
                    model.sspcab_activations["input"], model.sspcab_activations["output"]
                )
            ...
            # log loss
            l2_losses_m.update(l2_loss.item())
            ssim_losses_m.update(ssim_loss.item())
            ...
            if use_wandb:
                wandb.log({
                    ...
                    'train_l2_loss': l2_losses_m.val,
                    'train_ssim_loss': ssim_losses_m.val,
            ...
            if (step+1) % log_interval == 0 or step == 0:
                _logger.info(
                    'TRAIN [{:>4d}/{}] '
                    'Loss: {loss.avg:>6.4f} '
                    'L2 Loss: {l2_loss.avg:>6.4f} '
                    'SSIM Loss: {ssim_loss.avg:>6.4f} '
                    'L1 Loss: {l1_loss.avg:>6.4f} '
                    'Focal Loss: {focal_loss.avg:>6.4f} '
                    'LR: {lr:.6e} '
