...
class YOLOLoss(nn.Module):
    ...
    def __init__(self, focal_loss_alpha, focal_loss_gamma, loss_reg_weight, loss_cls_weight, topk_candidates, ignore_thresh, iou_thresh, num_classes=80):
        super().__init__()
        ...
        # ------------- Loss weight -------------
        self.loss_obj_weight = loss_reg_weight
        self.loss_cls_weight = loss_cls_weight

        # ------------- Matcher -------------
        self.matcher = UniformMatcher(topk_candidates)
    ...
    def loss_labels(self, pred_cls, tgt_cls, num_boxes):
        ...
        loss_cls = sigmoid_focal_loss(
            pred_cls, tgt_cls, self.alpha, self.gamma)
    ...
    def forward(self, outputs, targets):
        ...
        """
        # -------------------- Pre-process --------------------
        pred_box = outputs['pred_box'].detach().clone().cpu()
        pred_cls = outputs['pred_cls'].reshape(-1, self.num_classes).clone().cpu()
        anchor_boxes = outputs['anchors'].clone().cpu()
        B = len(targets)
        ...
        pos_ignore_idx = pos_ious < self.iou_thresh
        ...
        num_foreground = num_foreground.clamp(min=1).item()
        ...
        loss_labels = self.loss_labels(
            pred_cls[valid_idxs], gt_cls_target[valid_idxs], num_foreground)

        # -------------------- Regression loss --------------------
        tgt_boxes = torch.cat([t['boxes'][i] for t, (_, i) in zip(
            targets, indices)], dim=0).to(pred_cls.device)
        ...
        loss_bboxes = self.loss_bboxes(
            matched_pred_box, tgt_boxes, num_foreground)

        # total loss
        losses = self.loss_obj_weight * loss_bboxes + self.loss_cls_weight * loss_labels

        loss_dict = dict(
            loss_cls=loss_labels,
            loss_box=loss_bboxes,
            loss_obj=torch.tensor(0., dtype=torch.float32),
            losses=losses
        )
