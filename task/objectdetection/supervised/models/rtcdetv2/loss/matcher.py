...
class AlignedSimOTA(object):
    ...
    def __call__(self,
        ...
        pair_wise_ious_loss = - \
            torch.log(pair_wise_ious + 1e-8) * self.iou_weight

        # ----------------------------------- Classification cost -----------------------------------
        # select the predicted scores corresponded to the gt_labels
        pairwise_pred_scores = pred_cls.permute(1, 0)  # [M, C] -> [C, M]
        # [N, M]
        pairwise_pred_scores = pairwise_pred_scores[gt_labels.long(
        ), :].float()
        # scale factor
        scale_factor = (pair_wise_ious -
                        pairwise_pred_scores.sigmoid()).abs().pow(2.0)
        # cls cost
        ...
        assigned_labels = gt_labels.new_full(pred_cls[..., 0].shape,
                                             self.num_classes)  # [M,]
        assigned_labels[fg_mask_inboxes] = gt_labels[matched_gt_inds].squeeze(
            -1)
        ...
        assign_metrics = gt_bboxes.new_full(
            pred_cls[..., 0].shape, 0)  # [M, 4]
    ...
    def find_inside_points(self, gt_bboxes, anchors):
        ...
        anchors_expand = anchors.unsqueeze(0).repeat(
            num_gt, 1, 1)           # [N, M, 2]
        gt_bboxes_expand = gt_bboxes.unsqueeze(
            1).repeat(1, num_anchors, 1)  # [N, M, 4]
