...
class YOLOv4(nn.Module):
    def __init__(self, cfg, device, num_classes=20, conf_thresh=0.01, topk=100, nms_thresh=0.5, no_decode=False):
        ...
        # ------------------- Network Structure -------------------
        # backbone network
        self.backbone, feats_dim = build_backbone(cfg.backbone, cfg.pretrained)

        # neck
        self.neck = build_neck(
            cfg, in_dim=feats_dim[-1], out_dim=feats_dim[-1])
    ...
    def inference_single_image(self, x):
        ...
        for level, (feat, head) in enumerate(zip(pyramid_feats, self.non_shared_heads)):
            ...
            ctr_pred = (torch.sigmoid(
                reg_pred[..., :2]) * 3.0 - 1.5 + anchors[..., :2]) * self.stride[level]
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            # output dict
            outputs = {
                "pred_obj": all_obj_preds,        # List [B, M, 1]
